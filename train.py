...
def train():
    ...
    num_chunk_frames = model.features_extractor.num_output_frames(config.num_chunk_samples)
    ...
    logger = TensorBoardLogger("checkpoints_ILD/training_logs", name=None, version='logs')
    trainer = pl.Trainer(
        gpus=args.gpus,
    ...
    #trainer.test(model)
    checkpoint = torch.load('/home/ubuntu/Multichannel_dia/experiment5/checkpoints_ILD/epoch=109-val_der=0.224279.ckpt')
