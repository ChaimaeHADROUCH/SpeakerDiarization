...
def train():
    ...
    num_output_frames = model.features_extractor.num_output_frames(config.chunk_duration, config.sample_rate)
    ...
    checkpoint_callback = ModelCheckpoint(
        monitor="val_der",
        dirpath="checkpoints_CNN_stereo/",
    ...
    logger = TensorBoardLogger("checkpoints_CNN_stereo/training_logs", name=None, version='logs')
    ...
    checkpoint = torch.load('/home/ubuntu/Multichannel_dia/experiment5/checkpoints_CNN_stereo/epoch=144-val_der=0.202816.ckpt')
    
    model = StereoNet.from_config(config, writer, torch.tensor([0.5199999809265137]))
