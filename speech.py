...
#        self.root = root
    ...
    def _collate_fn(self, data: List[Dict]) -> Dict:
        # Batch tensors
        return {
            "mix": torch.vstack([sample["mix"] for sample in data]).float(), # shape (batch, channel, samples)
    ...
    def worker_init_fn(self, worker_id):
        ...
        self.dataset = worker_info.dataset  # the dataset copy in this worker process
        overall_start = self.dataset.start
        overall_end = self.dataset.end
        # configure the dataset to only process the split workload
        per_worker = int(math.ceil((overall_end - overall_start) / float(worker_info.num_workers)))
        worker_id = worker_info.id
        self.dataset.start = overall_start + worker_id * per_worker
        self.dataset.end = min(self.dataset.start + per_worker, overall_end) 
    '''
...
#        sr, audio = wavfile.read(audio_file)
    ...
    def __iter__(self):
        ...
        for i in range(len(self.annotations)):
            ...
            #end = time.monotonic()
            #print("time for rearrange in s ",end - start)
            ...
            for chunk in range(mix_chunks.shape[0]):
                ...
                if num_sources == 0:
                    ...
                    #logging.warning(msg)
                    raise ValueError("ERROR: TOO MANY SPEAKERS IN ANNOTATIONS (reprogram the code to fix it)")
                    #continue
