...
class Data_normalization():
    ...
    def get_normalization( self, batch_size:int = 32):
        ...
        for idx in range(number_batches):
            ...
            magnitude,ipd,ild = feature_computation.compute(chunk["mix"])
            ...
            ild =  reduce(ild, "batch channel freq time ->  channel freq", "sum")
            ...
            #ild
            '''
            for element in torch.isnan(ild):
                if (element == True).all():
                    continue
            '''
            ild_freq_sum += ild # Shape of ild_freq_sum = vector of F frequences
            ild_freq_sum_sq += ild**2 # Shape of ild_freq_sum_sq = vector of F frequences
       
        
        power_mag_sq = (1/total_samples)*mag_freq_sum_sq
        
        mag_mean = (1/total_samples)*mag_freq_sum
        
        mag_var = power_mag_sq - (mag_mean)**2

        ipd_mean = (1/(total_samples))*ipd_freq_sum
         
        power_ild_sq = (1/total_samples)*ild_freq_sum_sq
 
        ild_mean = (1/total_samples)*ild_freq_sum
    
        ild_var = power_ild_sq - (ild_mean)**2
        ...
        if isinstance(ild_mean, torch.Tensor):
            normalized_features['ild_mean'] = ild_mean.tolist()
        if isinstance(ild_var, torch.Tensor):
            normalized_features['ild_var'] = ild_var.tolist()

        with open('stats_stereo.json', 'w', encoding='utf-8') as json_file:
