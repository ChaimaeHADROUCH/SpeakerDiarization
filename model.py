...
class HybridSincNet(nn.Module):
    def __init__(
        ...
        self.conv1d_mag = nn.ModuleList()
        self.pool1d_mag = nn.ModuleList()
        self.norm1d_mag = nn.ModuleList()
        
        self.conv1d_ipd = nn.ModuleList()
        self.pool1d_ipd = nn.ModuleList()
        self.norm1d_ipd = nn.ModuleList()

        self.conv1d_mag.append(nn.Conv1d(65, 60, 5, stride=1)).to(device)
        self.pool1d_mag.append(nn.MaxPool1d(3, stride=3, padding=0, dilation=1)).to(device)
        self.norm1d_mag.append(nn.InstanceNorm1d(60, affine=True)).to(device)

        self.conv1d_mag.append(nn.Conv1d(60, 60, 5, stride=1)).to(device)
        self.pool1d_mag.append(nn.MaxPool1d(3, stride=3, padding=0, dilation=1)).to(device)
        self.norm1d_mag.append(nn.InstanceNorm1d(60, affine=True)).to(device)
        
        self.conv1d_ipd.append(nn.Conv1d(65, 60, 5, stride=1)).to(device)
        self.pool1d_ipd.append(nn.MaxPool1d(3, stride=3, padding=0, dilation=1)).to(device)
        self.norm1d_ipd.append(nn.InstanceNorm1d(60, affine=True)).to(device)

        self.conv1d_ipd.append(nn.Conv1d(60, 60, 5, stride=1)).to(device)
        self.pool1d_ipd.append(nn.MaxPool1d(3, stride=3, padding=0, dilation=1)).to(device)
        self.norm1d_ipd.append(nn.InstanceNorm1d(60, affine=True)).to(device)
    ...
    def forward(self, waveform: torch.Tensor) -> torch.Tensor:
        ...
        for (conv1d, pool1d, norm1d) in zip(self.conv1d_mag, self.pool1d_mag, self.norm1d_mag):
        ...
        for (conv1d, pool1d, norm1d) in zip(self.conv1d_ipd, self.pool1d_ipd, self.norm1d_ipd):
        ...
        if self.normalize_input:
            ...
            return torch.cat([magnitude_micro1, magnitude_micro2, ipd], dim=1)
...
class StereoNet(pl.LightningModule):
    ...
    def __init__(
        ...
        self.wav_norm1d = nn.InstanceNorm1d(2, affine=True)
    ...
    def forward(self, mix: torch.Tensor):
        ...
        normalized_input = self.wav_norm1d(mix)
       
        features = self.features_extractor.forward(normalized_input).float()
        
        features = rearrange(features, "batch feature frame -> batch frame feature")#[32, 276, 180]
