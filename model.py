...
class HybridSincNet(nn.Module):
    def __init__(
        ...
        self.conv1d.append(nn.Conv1d(65, 60, 5, stride=1)).to(device)
        self.pool1d.append(nn.MaxPool1d(3, stride=3, padding=0, dilation=1)).to(device)
        self.norm1d.append(nn.InstanceNorm1d(60, affine=True)).to(device)

        self.conv1d.append(nn.Conv1d(60, 60, 5, stride=1)).to(device)
        self.pool1d.append(nn.MaxPool1d(3, stride=3, padding=0, dilation=1)).to(device)
        self.norm1d.append(nn.InstanceNorm1d(60, affine=True)).to(device)
    ...
    def forward(self, waveform: torch.Tensor) -> torch.Tensor:
        ...
        magnitudes = rearrange(magnitudes, pattern).to(device)
        ipd = rearrange(ipd, pattern)

        #CNN
        
        for (conv1d, pool1d, norm1d) in zip(self.conv1d, self.pool1d, self.norm1d):
            magnitudes = F.leaky_relu(norm1d(pool1d(conv1d(magnitudes))))
...
class StereoNet(pl.LightningModule):
    ...
    def __init__(
        ...
        self.feature_extractor = HybridSincNet(fft_size)
    ...
    def forward(self, mix: torch.Tensor):
        ...
        features = self.features_extractor.forward(normalized_input).float() #[32, 65, 2501]
      
        features = rearrange(features, "batch feature frame -> batch frame feature")# [32, 276, 60]
        # Get common output
        outputs = self.lstm(features )[0]  # (batch, frames, features) [32, 276, 256]
     
        outputs = F.leaky_relu(self.linear1(outputs))#[32, 276, 128]
       
        outputs = torch.sigmoid(self.linear2(outputs)).transpose(1,2)#[32, 4, 276]
