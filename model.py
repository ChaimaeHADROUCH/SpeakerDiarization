...
class STFTFeatureExtractor(nn.Module):
    ...
    def forward(self, waveform: torch.Tensor) -> torch.Tensor:
        ...
        if self.normalize_input:
            ...
            self.mag_mean = self.mag_mean.reshape(-1)
            self.mag_var = self.mag_var.reshape(-1)
            self.ipd_mean = self.ipd_mean.reshape(-1)
            self.ild_mean = self.ild_mean.reshape(-1)
            self.ild_var = self.ild_var.reshape(-1)


            normalized_magnitude = torch.div(torch.sub(magnitude , self.mag_mean),(torch.sqrt(self.mag_var)))
            normalized_ild = torch.div(torch.sub(ild , self.ild_mean),(torch.sqrt(self.ild_var)))
            normalized_ipd = torch.sub(ipd ,self.ipd_mean)
         
             # Concatenate features, shape (batch, frames, features)
            return torch.cat([normalized_magnitude, normalized_ipd, normalized_ild ], dim=-1)
        else:                     
            return torch.cat([magnitude, ipd, ild], dim=-1)
...
class StereoNet(pl.LightningModule):
    ...
    def __init__(
        ...
        ild_mean = torch.Tensor(data["ild_mean"]).cuda()
        ild_var = torch.Tensor(data["ild_var"]).cuda()
        
        self.features_extractor = STFTFeatureExtractor( fft_size, "first", mag_mean=mag_mean, mag_var=mag_var,ipd_mean=ipd_mean, ild_mean=ild_mean, ild_var=ild_var ).to(device)
        
        num_features = self.features_extractor.num_output_features(num_channels)
        
       
        self.input_projection = nn.Linear(1028, 60)
    ...
    def forward(self, mix: torch.Tensor):
        ...
        normalized_features = self.features_extractor.forward(mix).float().cuda() #[32, 313, 1028]
      
        projected_normalized_features = F.leaky_relu(self.input_projection(normalized_features)) #[32, 313, 60]
 
        # Get common output
        outputs = self.lstm(projected_normalized_features )[0]  # (batch, frames, features)[32, 313, 256]
        
        outputs = F.leaky_relu(self.linear1(outputs))#[32, 313, 128]
        
        outputs = self.activation(self.linear2(outputs)).transpose(1,2)#[32, 4, 313]
