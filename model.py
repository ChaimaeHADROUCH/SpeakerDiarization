...
class STFTFeatureExtractor(nn.Module):
    def __init__(
        self,
        fft_size: int = 512,
        phase_diff_mode: Literal["first", "mean"] = "mean",
    ...
    def forward(self, waveform: torch.Tensor) -> torch.Tensor:
        ...
        if self.phase_diff_mode == "first":
            ipd = torch.sub(torch.angle(stft[:, 1:]) , torch.angle(stft[:, 0:1]))
        else:
            ipd = torch.sub(torch.angle(stft) , torch.angle(torch.mean(stft, dim=1, keepdim=True)))
        ipd = ipd % (2 * torch.pi) #32, 1, 129, 626 
        
        magnitude = stft.abs() #32, 2, 129, 626 
