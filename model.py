...
class STFTFeatureExtractor(nn.Module):
    ...
    def forward(self, waveform: torch.Tensor) -> torch.Tensor:
        ...
        magnitude_micro2 = stft[:, 0:2].abs()
        
         # Reshape magnitudes and IPD 
        pattern = "batch channel freq frame -> batch (channel freq) frame "
        magnitude_micro1 = rearrange(magnitude_micro1, pattern)
        magnitude_micro2 = rearrange(magnitude_micro2, pattern)
        ipd = rearrange(ipd, pattern)
        
        
        print("magnitude_micro1", magnitude_micro1.shape)
        print("magnitude_micro2", magnitude_micro2.shape)
        print("ipd", ipd.shape)
        ...
        if self.normalize_input:
            ...
            normalized_magnitude = torch.div(torch.sub(magnitudes , self.mag_mean),(torch.sqrt(self.mag_var)))
            ...
            return magnitude_micro1, magnitude_micro2, ipd
...
class StereoNet(pl.LightningModule):
    ...
    def __init__(
        ...
        self.sincnet = SincNet( 16000, stride = 10)
        ...
        self.linear1 = nn.Linear(256, hidden_size).to(device)
        self.linear2 = nn.Linear(hidden_size, num_speakers).to(device)
    ...
    def forward(self, stereo: torch.Tensor):
        ...
        output1_sincnet = self.sincnet(stereo_channel1).float().cuda()
     
        output2_sincnet = self.sincnet(stereo_channel2).float().cuda()
    
        concatenated_outputs_sincnet  = torch.cat([output1_sincnet, output2_sincnet], dim=1)
        
        concatenated_outputs_sincnet = rearrange(concatenated_outputs_sincnet, "batch feature frame -> batch frame feature")
    
        #projected_normalized_features = F.relu(self.input_projection(normalized_features))
        
        # Get common output
        outputs = self.lstm( concatenated_outputs_sincnet )[0]  # (batch, frames, features)
